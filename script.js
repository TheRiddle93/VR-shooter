...
const teleport = () => {
  ...
  const bullet = document.createElement("a-torus"); // for more shapes go to: code changers.com/lessons/vr -> shapes tab
  let pos = myCamera.getAttribute("position");
  bullet.setAttribute("position", pos);
  bullet.setAttribute("velocity", getDirection(myCamera, 30));
  bullet.setAttribute("dynamic-body", true);
...
document.onkeydown = (event) => {
  if (event.which == 32) {
    shoot(); // 32 = the spacebar key
  } else if (event.which == 67) {
    teleport(); // 67 = the c key, javascript keycodes for more key numbers
  }
};
